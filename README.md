...
Simply copy [`imgui_impl_dx9.h`](./imgui_impl_dx9.h), [`imgui_impl_dx9.cpp`](./imgui_impl_dx9.cpp), [`imgui_impl_xbox360.h`](./imgui_impl_xbox360.h) and [`imgui_impl_xbox360.cpp`](./imgui_impl_xbox360.cpp) into your project, and refer to the [example app](./examples/title/main.cpp) or [example DLL](./examples/dll/main.cpp) to see how to use them.
...
- Unloading the XInput DLL was removed from the shutdown function because this backend never loads it.
...
- The shutdown function is only meant to be called in system DLLs, so the release of the `D3DDevice` was omitted because system DLLs don't own the `D3DDevice`, the games they inject code into do.
...
- The shutdown functions are provided but not used in the [example app](./examples/title/main.cpp) because the main loop isn't supposed to be stopped by the app itself. When switching games, the system switches context to the kernel immediately without waiting for the main function to return. The functions are provided for the [example DLL](./examples/dll/main.cpp) because system DLLs continue to run after switching games.
...
- When using the [example DLL](./examples/dll/main.cpp), launching the dashboard a second time will crash because of a segfault in a `malloc` during the ImGui initialization. The reason for this segfault hasn't been found yet.

## Acknowledgements

- [ImGui_Dash_Hook_POC](https://github.com/Byrom90/ImGui_Dash_Hook_POC) - What inspired the [example DLL](./examples/dll/main.cpp).
